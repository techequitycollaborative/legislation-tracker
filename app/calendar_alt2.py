...
# -*- coding: utf-8 -*-
"""
Calendar Page -- alternative version #2
...
UPDATE: This page now features clickable calendar event blocks.
...
# Build pop-up for clicked events
@st.dialog("📋 Event Details")
def show_event_dialog(event):
    # Grab extended properties
    extended_props = event.get("extendedProps", {})
    
    # Get and transform start and end dates
    start = event.get('start', 'N/A')
    start_clean = pd.to_datetime(start).strftime("%B %d, %Y")
    end = event.get('end', 'N/A')

    # Display event title
    st.subheader(event.get("title", "Untitled Event"))
    
    # If end date exists, display start and end
    if end != 'N/A':
        # Subtract one day from end date (streamlit-calendar needs the real end date to be 1 day 
        # ahead for display purposes, but we want to show the actual end date in the dialog)
        end_clean = (pd.to_datetime(end) - pd.Timedelta(days=1)).strftime("%B %d, %Y")
        
        st.write(f"**Start:** {start_clean}")
        st.write(f"**End:** {end_clean}")
    
    # If no end date, just display start date as 'Date'
    else: 
        st.write(f"**Date:** {start_clean}")
    
    # Display additional bill details if they exist
    if extended_props:
        st.divider()
        type = extended_props.get("type", "N/A")
        bill_number = extended_props.get("billNumber", "N/A")
        bill_name = extended_props.get("billName", "N/A")
        status = extended_props.get("status", "N/A")
        date_introduced = extended_props.get("date_introduced", "N/A")

        if type != 'Legislative':
            st.markdown("### ***Additional Bill Details:***")
            ...
            if date_introduced != "N/A":
                date_clean = pd.to_datetime(date_introduced).strftime("%B %d, %Y")
                st.write(f"- **Date Introduced:** {date_clean}")
        else: 
            st.write("")

# Handle event click — store in session state
if calendar_widget and calendar_widget.get("callback") == "eventClick":
    st.session_state.clicked_event = calendar_widget["eventClick"]["event"]

# Open dialog if an event is stored
if st.session_state.clicked_event:
    show_event_dialog(st.session_state.clicked_event)
